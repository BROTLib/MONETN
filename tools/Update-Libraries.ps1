# Update-Libraries.ps1 (MONETN)
#
# Manually-run: checks GitHub for the latest release of each library MONETNRuntime
...
#
# Note MONETcommon is deliberately excluded: MONETNRuntime vendors local copies of
# MONETcommon's FBs rather than referencing the library (see
# IAG/specs/design/monetcommon-monetn-monets-unification-plan.md), so there is
# nothing for this script to update there until that unification lands.

param(
    # Exactly the custom libraries MONETNRuntime.plcproj references via
    ...
    [string[]]$Libraries = @("AstroBROT", "BROTLib", "HalfBROT", "MONETRoof"),
    [string]$TcProfile
)
